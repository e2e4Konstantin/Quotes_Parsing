...
def read_machines_tables(data: SourceData):
    """
...
def get_resource(data: SourceData, row: int, parent_table_index: int) -> Resource:
    ...
    resource_item = Resource(
        ...
        okp=data.get_cell_str_value(row, data.get_column_number("G")),
...
def read_machines(data: SourceData):
    """ Обрабатывает данные по Машинам из data. Записывает машины. """
    read_machines_tables(data)
    if len(resource_tables) > 0:
        table_rows: list[int] = [x.row_table for x in resource_tables]
        table_rows.sort()
        print(f"\nдля Машин прочитано таблиц: {len(resource_tables)}")  # , {table_rows}
            ...
            if check_by_list(data, row_i, ['B', 'C', 'D'], "resource"):
                ...
                if is_data_on_right_side(data, row_i, "J"):
                    ...
                    if owner_table_row >= 0:
                        ...
                        print(f"для машины на строке {row_i} не найдена таблица.")
                else:
                    resource_without_data += 1

        print(f"\nОбработано строк: {data.row_max}")
        print(f"Обработано Машин: {resource_with_data}")
        print(f"Записано машин: {len(resource_data)}")
        print(f"Машин без параметров/атрибутов: {resource_without_data}")
    else:
        print(f"В Машинах не найдено ни одной таблицы с атрибутами/параметрами")
